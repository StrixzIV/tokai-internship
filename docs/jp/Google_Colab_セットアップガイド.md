...
- **Step 3a** のコードセルを実行し、生の未処理の瞳孔径をプロットします。
- このセルでは、経時的な生の瞳孔径のみを表示し、瞬き中またはアイトラッカーのロスト時にゼロに急降下している箇所を確認できます。背景には青色LED刺激のアクティブブロックが重ねて表示されます。

### ステップ 3b: 補間クオリティの視覚化（生データとクリーニングデータの重ね合わせ）
- **Step 3b** のコードセルを実行し、生データとPCHIP補間されたデータを比較します。
- **検証ポイント**:
